...
deficit compared to Chromium-based alternatives such as Vanadium. Their
core claim regarding the absence of Android’s `isolatedProcess`
sandboxing in GeckoView remains accurate and is acknowledged in this
analysis. However, several subsidiary claims conflate distinct
mitigation layers (pre-compromise versus post-compromise) and fail to
account for threat-model dependencies that alter the security calculus.
This paper evaluates these claims through a multi-layered
threat-modeling lens grounded in publicly available primary sources. It
examines the architectural evolution of GeckoView on Android, including
the January 2026 shipping and subsequent rollback of Project Fission
(Site Isolation) in Firefox 147 (now at Firefox 152, July 2026), which
remains disabled on release and beta channels due to unresolved crash
bugs (Section 2.3). It covers structural pre-compromise defenses
provided by Firefox’s industry-leading Rust adoption, WebExtension
isolation architecture, and declarative content-blocking engine. It
looks at the intersection of privacy controls with exploit-chain
disruption, including Total Cookie Protection and Enhanced Tracking
Protection. And it considers the systemic security risk of a Chromium
monoculture. The analysis finds that categorical dismissal of either
engine family is unsupported by current evidence, and that browser
selection is not a binary metric of "secure versus insecure" but an
alignment with a specific threat model.
...
#strong[This creates an asymmetric risk profile.] Chromium constructs a
tighter post-compromise sandbox (through `isolatedProcess` and strict
site isolation) but simultaneously expands its pre-compromise attack
surface by integrating aggressive device APIs. Firefox scales down the
sandbox thickness on certain platforms (notably Android, where
`isolatedProcess` is absent and Fission remains disabled on release
channels), but balances this by strictly gating extension protocol
access \[32\] and rejecting entire classes of web-exposed attack
surface.
...
The practical implication is that the comparison cannot be reduced to
sandbox thickness alone. Chromium’s larger API surface creates a larger
vulnerability pool whose exploitation, even if well-contained, still
...
The practical implication is that Chromium’s memory safety strategy is
multi-layered rather than language-dependent. The absence of widespread
Rust adoption is partially compensated by a heterogeneous portfolio of
mitigation techniques applied across the C++ attack surface. A fair
comparison of pre-compromise defenses must account for both Firefox’s
Rust adoption and Chromium’s mitigations portfolio, rather than treating
Rust coverage as the sole metric.
