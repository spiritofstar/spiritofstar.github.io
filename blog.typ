...
== Abstract
<abstract>
GrapheneOS advises against Gecko-based browsers like Firefox, citing a
security gap compared to Chromium-based alternatives such as Vanadium.
Their core claim about the absence of Android’s `isolatedProcess`
sandboxing in GeckoView is accurate and acknowledged here. Several
subsidiary claims mix up distinct mitigation layers (pre-compromise
versus post-compromise) and don’t account for threat-model dependencies
that change the security calculus. This paper evaluates those claims
through a multi-layered threat-modeling lens using publicly available
sources. It covers the architectural evolution of GeckoView on Android,
including the January 2026 shipping and subsequent rollback of Project
Fission (Site Isolation) in Firefox 147, which remains disabled on
release and beta channels due to unresolved crash bugs (Section 2.3). It
covers structural pre-compromise defenses from Firefox’s Rust adoption,
WebExtension isolation, and declarative content-blocking. It looks at
privacy controls intersecting with exploit-chain disruption. And it
considers the systemic security risk of a Chromium monoculture. The
analysis finds that categorical dismissal of either engine family isn’t
supported by current evidence. Browser selection is not a binary "secure
versus insecure" metric but alignment with a specific threat model.
...
that have since been partially addressed continue to circulate as
authoritative guidance without reference to current implementation
status.

This paper does two things. One, provide an evidence-based assessment of
the security properties of both Chromium-based and Gecko-based browser
architectures on Android. Distinguish between verified architectural
properties, documented limitations, and areas where public evidence is
incomplete. Two, offer a claim-by-claim examination of GrapheneOS’s
published position. Identify where assertions remain valid, where
they’ve been superseded by developments, and where they reflect
divergent threat-model priorities rather than objective security
deficits.
...
The statement "Firefox does not have internal sandboxing on Android"
treats the absence of one specific mechanism as the complete absence of
sandboxing. Three developments have altered this assessment:
...
+ #strong[Project Fission (Site Isolation), shipped, then rolled back.]
  ...
  isolation. This is different from Chromium’s site isolation model and
  means the cross-origin exfiltration protection described in the paper
  is currently unavailable on release Firefox for Android.
...
Even if Fission were active, it does not replicate the
...
cross-origin data exfiltration even with side-channel attacks. But it
does not provide the same post-exploit kernel-level containment. The
relationship between Fission and `isolatedProcess` is complementary
rather than substitutive. Fission’s current `ISOLATE_HIGH_VALUE`
strategy is also narrower than Chromium’s full site isolation, it only
isolates high-value origins.

=== 2.4 Is `isolatedProcess` the Complete Picture?
<is-isolatedprocess-the-complete-picture>
Does the absence of `isolatedProcess` sandboxing settle the question of
GeckoView’s security posture? Three reasons it does not:
...
<capability-expansion-vs.-containment-rigor>
The security calculus between Chromium and GeckoView is further
complicated by each project’s strategy for expanding the web platform’s
native device access APIs. This section examines the trade-off between
surface area expansion and sandbox rigor.

Chromium has aggressively pushed the boundaries of what web-exposed
interfaces can access. Key examples include:

- #strong[Direct Sockets API.] Allows web applications to establish
  direct TCP and UDP communications, bypassing HTTP abstractions
  entirely. While gated behind permissions and secure contexts, this
  introduces raw networking access into the browser’s attack surface. A
  vulnerability in the Direct Sockets implementation could expose
  low-level network access to an attacker who has achieved code
  execution in a renderer.
- #strong[Isolated Web Apps (IWAs).] A new application model that blurs
  the line between web pages and native applications, granting packaged
  web apps elevated capabilities including raw socket access, file
  system write access, and system API exposure \[31\].
- #strong[HID and USB device access.] APIs that allow web applications
  to enumerate and communicate directly with hardware peripherals. These
  represent a significant expansion of the browser’s trusted computing
  base into hardware-interface code traditionally reserved for native
  applications.
- #strong[File System Access API.] Gives web applications read and write
  access to the local file system outside the browser’s storage sandbox.
  While user-gated through file pickers, the API expands the blast
  radius of a compromised renderer to include local file system data.

These capabilities expand the browser’s trusted computing base at the
API layer. Even with permission gates and sandboxing, each new API
represents code paths that must be correct. As the Chromium security
team’s own data shows, the majority of severe vulnerabilities are
memory-safety bugs in C++ code \[14\]. Every additional thousand lines
of C++ API implementation expands the pool of potential vulnerabilities,
regardless of the sandbox quality.
...
of the more invasive device APIs. This is a deliberate product
philosophy that prioritizes capability gating over capability expansion.
The reasoning is straightforward: an API that does not exist in the
codebase cannot be exploited.

The result is an asymmetric risk profile. Chromium builds a tighter
post-compromise sandbox (through `isolatedProcess` and strict site
isolation) but simultaneously expands its pre-compromise attack surface
with aggressive device APIs. Firefox scales down sandbox thickness on
certain platforms (Android, where `isolatedProcess` is absent and
Fission remains disabled on release channels), but balances this by
gating extension protocol access \[32\] and rejecting entire classes of
web-exposed attack surface.

Bug 2034168 illustrates this philosophy in the extension context: as of
Firefox 153 (July 2026), extensions can no longer access local files by
default. The `file:` scheme access is gated behind an explicit "Access
local files on your computer" permission, entirely separate from the
broader "Access your data for all websites" permission \[32\]. An
extension that can inject content scripts into every website cannot
automatically read local files opened in the browser, because those two
capabilities are now separated at the permission model level. This is
the same principle applied at the extension API layer: capability gating
as a structural defense, not merely post-compromise containment.

The comparison cannot be reduced to sandbox thickness alone. Chromium’s
larger API surface creates a larger vulnerability pool. Even
well-contained exploitation of that pool still threatens cross-origin
data confidentiality. Firefox’s smaller API surface creates a smaller
vulnerability pool, partially compensating for weaker kernel-level
containment on Android. Neither approach is obviously superior. The two
projects made different trade-offs at different layers. A fair
comparison must account for both sides.
...
Google’s own security research has consistently found that roughly 70%
of critical-severity vulnerabilities in Chromium are memory-safety bugs
\[14\]. Microsoft’s Security Response Center has reported comparable
figures across its products \[15\]. By eliminating these vulnerability
...
#strong[Chromium’s Rust adoption remains limited and experimental.] As
...
<chromiums-memory-safety-and-mitigation-architecture>
The Rust-focused comparison in Section 3.1 presents an incomplete
picture of Chromium’s defense-in-depth. While Chromium’s codebase
remains predominantly C++, the project deploys multiple structural
mitigations that reduce memory corruption vulnerability density and
exploitability:

#strong[V8 Sandbox (sandboxed execution for JIT-compiled code).] V8,
Chromium’s JavaScript engine, implements a memory sandbox within the
address space of the renderer process. The V8 sandbox reserves a virtual
address region (typically 1 TB on 64-bit systems) and constrains all
JIT-compiled code and V8 heap allocations to this region. Pointers
between sandboxed and non-sandboxed memory are encoded as offsets
relative to the sandbox base, making it structurally impossible for
JIT-generated code to directly reference memory outside the sandbox
region even in the presence of a JIT corruption vulnerability \[34\].
This is a distinct mitigation from the kernel-level `isolatedProcess`
sandbox and operates at the memory-safety layer.

#strong[Oilpan garbage collection for the DOM.] Chromium’s DOM objects
are managed by Oilpan, a tracing garbage collector integrated into the
Blink rendering engine. Oilpan eliminates use-after-free vulnerabilities
in DOM-manipulation code paths by providing precise reachability
tracking and ensuring that object lifetimes are determined by reference
reachability rather than manual reference counting \[35\]. This is
architecturally significant because DOM-manipulation code has
historically been a dense source of use-after-free vulnerabilities in
browser engines. Firefox’s SpiderMonkey uses a combination of reference
counting and cycle collection for DOM object management, which provides
memory safety guarantees that are overlapping with but not identical to
Oilpan’s.

#strong[Type-checked IPC (Mojo).] Chromium’s Mojo IPC system enforces
type-safe message passing between processes. Interface definitions are
...
#strong[PartitionAlloc.] Chromium’s memory allocator provides hardened
allocation with features including: dedicated per-process partitions
(array/string/buffer allocations isolated from general-purpose
allocations), Bucket-based freelist entropy injection (ASLR within the
heap), and MiraclePtr (use-after-free quasi-reference-counting via
reference stability) \[37\]. These mitigations raise the exploitation
difficulty for heap corruption vulnerabilities even when the underlying
bug is present.

#strong[Type-based CFI.] Chromium ships with Clang’s Cross-DSO CFI
(Control Flow Integrity) enabled on supported platforms, validating
indirect call targets against their declared types at runtime. This
defeats type-confusion-based exploitation techniques that are a common
vector for gaining code execution from memory corruption primitives
\[14\].

#strong[Memory Tagging Extension (MTE).] On ARMv9 hardware, Chromium
supports MTE-based heap tagging, which probabilistically detects spatial
and temporal memory safety violations at the hardware level. This is
noted in Section 3.4 as a shared mitigation, but Chromium’s
implementation is more mature, with per-partition MTE tags integrated
into PartitionAlloc \[37\].

Chromium’s memory safety strategy is multi-layered rather than
language-dependent. The absence of widespread Rust adoption is partially
compensated by a portfolio of mitigation techniques applied across the
C++ attack surface. A fair comparison of pre-compromise defenses must
account for both Firefox’s Rust adoption and Chromium’s mitigation
portfolio.
...
+ #strong[Cryptographic side channels require language-independent
  ...
  address side-channel leakage. Firefox uses NSS (Network Security
  Services) for cryptography, with growing Rust components, while
  Chromium uses BoringSSL (a Google fork of OpenSSL). Both libraries
  require identical care in side-channel-resistant implementation.
...
Google applies comparable techniques across Chromium and Android,
including AI-guided fuzzing, automated vulnerability discovery via
OSS-Fuzz, and ML-based patch analysis. Both teams operate in the same
dual-use environment. Mozilla’s Anthropic collaboration represents an
investment in hardening methodology that both vendors will need to
sustain as AI-assisted offensive capabilities mature.
...
argument for Chromium’s sandbox assumes renderer compromise is
inevitable and post-compromise containment is critical. But the
...
Post-exploit mitigations (both software-level like `isolatedProcess` and
hardware-level like PAC) are important but interdependent. Hardware
mitigations make certain classes of sandbox escape more difficult, but
they cannot compensate for a structurally higher vulnerability density
in the rendering engine itself.
...
memory safety bugs dominate the CVE inventories of both engines, and (b)
Mozilla’s Rust components have a clean track record since shipping, but
the sample size and public documentation are insufficient for a
quantitative cross-browser comparison. Architectural arguments about
Rust’s security value (Section 3.1-3.3) should be evaluated in light of
this limited empirical basis.
...
\[1\]. This characterization mixes up distinct security mechanisms and
overlooks structural properties of Firefox’s extension architecture.
...
The security implication is that a compromised extension cannot easily
pivot to compromise other extensions. A compromised content-blocking
extension like uBlock Origin cannot exfiltrate data from a password
manager extension’s storage. This blast radius containment is a
structural security property of the extension architecture.
...
+ #strong[Fingerprinting risk.] Custom extensions and configurations
  increase browser distinctiveness. The marginal fingerprinting cost of
  a widely-used extension with default settings is lower than
  GrapheneOS’s framing suggests.
...
+ #strong[Larger extension API surface.] Maintaining support for the V2
  ...
  extension. This is a real trade-off.
...
GrapheneOS’s position falls firmly in the former camp \[1\]. For users
who deploy extensions, network-layer content interception as a
pre-compromise defense justifies the API surface exposure. For users who
do not use extensions, the API surface difference is irrelevant.

#line()

== 5. Privacy Architecture Overlap with Security Models
<privacy-architecture-overlap-with-security-models>
Security and privacy intersect at the reconnaissance phase of targeted
exploitation. Privacy controls that disrupt device fingerprinting,
cross-site tracking, and behavioral profiling directly impede an
attacker’s ability to identify and target specific individuals.
...
cannot deliver a tailored exploit. For users whose primary risk is
targeted exploitation enabled by data-broker profiling (rather than
state-sponsored zero-click attacks), privacy controls serve a real
security function.
...
<engine-diversity-vs.-aggregate-attack-surface>
GrapheneOS objects that GeckoView is not a WebView implementation, so
Firefox on Android must be deployed alongside the platform’s
Chromium-based WebView, creating "the remote attack surface of two
separate browser engines instead of only one" \[1\]. This objection is
addressed in detail in the claim-by-claim analysis (Section 7.5). In the
context of monoculture risk, the trade-off can be stated concisely: the
dual-engine state is an Android platform constraint, not a Firefox
deficiency. The marginal attack surface of adding GeckoView must be
weighed against the monoculture risk reduction that engine diversity
provides.
...
dispute \[1\]. The `isolatedProcess` attribute is a declarative manifest
flag, and its absence represents a deliberate or resource-constrained
decision by Mozilla. The earlier version of this paper categorized this
claim as "partially outdated," which overstated the degree to which the
architecture had changed. #strong[Correction: the core `isolatedProcess`
claim remains fully substantiated.]

The definitional question is whether "internal sandboxing" requires
`isolatedProcess` specifically, or whether other forms of process
isolation (multi-process architecture, Fission’s origin-level process
assignment on nightly/developer channels) qualify as sandboxing. This
paper takes the broader definition, which GrapheneOS disputes. The
reader should evaluate which definition aligns with their threat model:

- If sandboxing is defined as kernel-level UID isolation via
  `isolatedProcess`, GrapheneOS’s claim is correct and has not aged.
- If sandboxing is defined more broadly to include any form of
  process-level privilege separation, GeckoView provides process
  isolation without kernel-level UID sandboxing, and the two projects
  are using different definitions.

The first version of this paper did not make this definitional
distinction clearly, which created an impression of dismissing
GrapheneOS’s accurate claim. The claim is correct within GrapheneOS’s
definitional framework, and this paper should have stated that
explicitly.
...
GrapheneOS’s framing that Firefox is "much more vulnerable" mixes up a
difference in post-compromise architecture with a difference in overall
exploit risk. These are distinct metrics, and the available evidence
does not support the categorical claim.
...
+ #strong[The AntiVirus analogy is inapt.] GrapheneOS equates
  ...
  AntiVirus scans files on disk after delivery. WebExtension content
  blocking intercepts requests at the network layer #emph[before] any
  payload reaches the rendering engine.
...
+ #strong[Content blocking reduces attack surface directly, independent
  ...
  engine. This operates as a security measure regardless of the
  browser’s anti-fingerprinting quality.
...
\[Online\]. Available:
#link("https://www.mozilla.org/en-US/firefox/android/152.0/releasenotes/")

\[27\] Bug 2011886 - Switch off isolated processes by default. Mozilla
Bugzilla.
#link("https://bugzilla.mozilla.org/show_bug.cgi?id=2011886")[https://bugzilla.mozilla.org/show\_bug.cgi?id\=2011886]

\[28\] Bug 2011319 - Firefox for Android frequently and randomly going
back to the previous page. Mozilla Bugzilla.
#link("https://bugzilla.mozilla.org/show_bug.cgi?id=2011319")[https://bugzilla.mozilla.org/show\_bug.cgi?id\=2011319]

\[29\] Bug 2012435 - Content process crashes when isolating a site with
Fission. Mozilla Bugzilla.
#link("https://bugzilla.mozilla.org/show_bug.cgi?id=2012435")[https://bugzilla.mozilla.org/show\_bug.cgi?id\=2012435]

\[30\] Bug 2003658 - Make Fission + SHIP default in 147. Mozilla
Bugzilla.
#link("https://bugzilla.mozilla.org/show_bug.cgi?id=2003658")[https://bugzilla.mozilla.org/show\_bug.cgi?id\=2003658]

\[31\] Chrome Developers, "Isolated Web Apps." \[Online\]. Available:
#link("https://developer.chrome.com/docs/extensions/reference/manifest/isolated-app/")
. Accessed: Jul. 2026.

\[32\] Bug 2034168 - Restrict extensions from reading local file data
without specific permission. Mozilla Bugzilla.
#link("https://bugzilla.mozilla.org/show_bug.cgi?id=2034168")[https://bugzilla.mozilla.org/show\_bug.cgi?id\=2034168]

\[33\] Chromium Security Architecture.
#link("https://chromium.googlesource.com/chromium/src/+/master/docs/security/overview.md").
Accessed: Jul. 2026.

\[34\] V8 Sandbox Design Document.
#link("https://docs.google.com/document/d/1FM4fQmIhEqPG8uGp5o9A-mnPB5BOeScZYpkHjo0KKA8").

\[35\] Oilpan Design Document. Chromium.
...
\[36\] Mojo IPC Documentation. Chromium.
#link("https://chromium.googlesource.com/chromium/src/+/main/mojo/README.md")

\[37\] PartitionAlloc Design Document. Chromium.
